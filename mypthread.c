			...
			else
			{
				...
				else
				{
					...
					if(json_checker(gw_sn)==0 && sn_fd_len >0)
					{
						...
						create_secret_str(str_json,this_gw_sn->valuestring,my_gw_mac);
						send(cd,str_json,strlen(str_json),0);
...
void pthread_v_send(uint8_t *u_data)
{
	...
	else if(u_data[10] == 0x40)//设备描述
	{
		...
		{
			gw_bind_self_mac(u_data_str);//绑定主网关mac
			dev_descrip_u(i,u_data_len,u_data_str);
		}
	}
	else if(u_data[10] == 0x41)//son设备描述
	{
		if(go_net_flag==1 && NET_FLAG)
			dev_descrip_u(i,u_data_len,u_data_str);
	}
	...
	else if(u_data[10] == 0xf2)
	{
		gw_mac_bind_dev_mac(u_data_str);//绑定子网关mac
	}
	else if(u_data[10] == 0xf3)
	{
		printf("u_data_str : %s\n",u_data_str);
		gw_bind_self_mac(u_data_str);//绑定主网关mac
	}
			...
			while(p!=NULL)
			{
				...
				if( time_diff > 2000 && p->now_times > 0)
				{
					...
					else
					{
						...
						hex_to_str(p->cmd+2,dev_mac_resend,8);
						dev_mac_resend[16] = '\0';
						find_mac_and_send(dev_mac_resend,p->cmd,cmd_len);
						usleep(200000);
					}
				}
				p = p->next;
			...
			if(status_flag)
			{
				...
				if(gw_mac_dev != NULL)
				{
					if(gw_mac_dev->child != NULL)
					{
						...
						{
							if(strcmp(tem_mac->valuestring,while_find->string)==0)
							{
								cJSON *bind_gw_mac = cJSON_GetObjectItem(gw_mac_dev,tem_mac->valuestring);
								if(strcmp(bind_gw_mac->valuestring,gw_mac)==0)
								{
									if(status_flag==1)
									{
										memset(final_cmd_stastus,0,16);
										cmd_mix_get_stastus1(tem_mac->valuestring,tem_port->valuestring,"00",final_cmd_stastus);
										if(!strcmp(gw_mac,my_gw_mac))
										...
										else
										{
											...
											while( p )
											{
												if( p->flag && !strcmp(gw_mac,p->mac))
												{
													if(send_to_son_gw_have(final_cmd_stastus,16,p->fd_net) == -1)
														p->flag = 0;
													break;
												}
												p = p->next;
							...
							if(while_find == NULL)
							{
								if(status_flag==1)
								{
									memset(final_cmd_stastus,0,16);
									cmd_mix_get_stastus1(tem_mac->valuestring,tem_port->valuestring,"00",final_cmd_stastus);
									sign_mac_zt(final_cmd_stastus);
									usart_send(fd,final_cmd_stastus,16);
									send_to_son_gw_no(final_cmd_stastus,16);
									usleep(200000);
								}
							}
						}
